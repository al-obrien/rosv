...
Various helper function assist in the administration of [Posit Package Manager](https://packagemanager.posit.co/client/#/) or similar services. Packages can be routinely examined for new vulnerabilities which aide in the creation and updating of curated repositories as well as setting block lists.
...
Provide a package name and related ecosystem to fetch any identified vulnerabilities.

```{r, eval = FALSE}
osv_query('dask', ecosystem = 'PyPI')
```

Multiple packages can be queried at the same time and can be mixed between ecosystems.
